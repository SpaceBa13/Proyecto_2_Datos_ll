...
func _physics_process(delta):
	...
	var own_position_h = tile_map.local_to_map(global_position)
	var player_position_h = tile_map.local_to_map(player.global_position)
	var heuristic = Astar_path.heuristic(own_position_h, player_position_h)
...
func animate(direction: Vector2):
	if direction.x > 0 and death == true:
